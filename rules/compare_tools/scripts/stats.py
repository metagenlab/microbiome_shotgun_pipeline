...
def get_precision_recall_f1(true_tb, tool_tb, rank, tool_name, superkingdom):
    ...
    names = list(true_tb['species'])
    taxids = list(true_tb['species_taxid'])
    taxid2names = dict(zip(taxids, names))
    true_list = list(true_tb.groupby([f'{rank}_taxid'], as_index=False).sum()['species_taxid'])
    tool_list = list(tool_tb.groupby([f'{rank}_taxid'], as_index=False).sum()['species_taxid'])
