...
p <- ggplot(rpkm_table, aes(sample, description)) + geom_tile(aes(fill = RPKM)) + scale_fill_gradient(low = "white", high = "steelblue", na.value = "grey50")
p <- p + geom_text(aes(label = round(RPKM, 1)), size=1) 
p <- p + theme(axis.text.x = element_text(angle = 90))
p <- p + facet_grid( . ~ group_2, scales="free")
p <- p + theme_bw() + theme(axis.text.x = element_text(angle = 90)) 
ggsave(snakemake@output[[5]], p, height=49, width=10)
...
p <- p + facet_grid( . ~ group_2, scales="free")
p <- p + theme_bw() + theme(axis.text.x = element_text(angle = 90)) 
ggsave(snakemake@output[[8]], p, height=12, width=10)