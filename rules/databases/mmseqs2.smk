...
rule execute_mmseqs2_map:
    ...
    params:
        evalue = 0.001,
    log:
        "samples/{sample}/{data_type}/{db_name}/mmseqsmap_merged_combined.log"
    output:
        "samples/{sample}/{data_type}/{db_name}/mmseqsmap_merged_combined_raw.m8"
    shell:
        """
        mmseqs map --threads {threads} -e {params[0]} {input[1]} {input[0]} {output[0]} $(dirname {output[0]})/tmp >> {log}
        """
